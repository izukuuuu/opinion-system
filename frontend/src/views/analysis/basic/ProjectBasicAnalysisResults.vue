<template>
  <div class="space-y-4">
        ...
        <div class="flex items-center gap-2">
          <div ref="manualInputRef" class="relative" v-if="viewSelection.topic">
            ...
            <div v-if="showManualInput"
              class="absolute right-0 top-full z-50 mt-2 w-80 rounded-2xl border border-soft bg-surface p-4">
              <form class="space-y-3" @submit.prevent="handleManualSubmit">
                ...
                <div class="flex flex-wrap gap-3 text-sm">
                  ...
                  <button type="button" class="btn-ghost" :disabled="loadState.loading || !viewSelection.topic"
                    @click="loadResults()">
          ...
          <button type="button"
            class="btn-secondary inline-flex items-center gap-2 whitespace-nowrap px-4 py-2 text-xs font-semibold sm:text-sm"
            :disabled="historyState.loading || loadState.loading || !viewSelection.topic" @click="refreshHistory">
            <ArrowPathIcon class="h-4 w-4" :class="historyState.loading ? 'animate-spin' : ''" />
        ...
        <div class="flex flex-col gap-2 md:flex-row md:items-end md:gap-4">
          <label class="flex-1 space-y-2 text-secondary">
            ...
            <select v-model="viewSelection.topic" class="input"
              :disabled="topicsState.loading || !selectableTopicOptions.length" required>
          ...
          <div ref="timeRangeRef" class="relative" v-if="viewSelection.topic">
            <label class="space-y-2 text-secondary">
              ...
              <button type="button"
                ...
                :class="showTimeRange ? 'border-brand-soft ring-1 ring-brand-soft/80' : ''" @click="toggleTimeRange">
                ...
                <ChevronDownIcon class="h-4 w-4 shrink-0 text-muted transition-transform"
                  :class="showTimeRange ? 'rotate-180' : ''" />
              </button>
            </label>

            <Teleport to="body">
              <div v-if="showTimeRange" class="fixed z-[9999] rounded-2xl border border-soft bg-surface p-4"
                ...
                <div v-else-if="historyState.error"
                ...
                <div v-else-if="analysisHistory.length" class="grid gap-2.5 lg:grid-cols-2">
                  <button v-for="record in analysisHistory" :key="record.id" type="button"
                    ...
                    :class="record.id === selectedHistoryId
                        ? 'border-brand-soft bg-brand-soft text-brand-700 ring-1 ring-brand-soft'
        ...
        <template v-if="analysisSummary">
          <div class="border-t border-soft pt-4">
            <dl class="flex flex-wrap items-center gap-4 text-sm">
              <div class="flex items-center gap-2">
                <span
                  class="inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-brand-600/10 text-brand-600">
    ...
    <section v-if="aiMainFinding || aiSummaryItems.length" class="card-surface space-y-4 p-5 sm:p-6">
      ...
      <div v-if="aiMainFinding" class="rounded-2xl border border-brand-soft bg-brand-soft/30 px-4 py-3 text-sm">
        <div class="flex items-start gap-2">
          <span
            class="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand-600/10 text-brand-600">
          ...
          <div class="space-y-1">
            ...
            <p v-if="aiMainFinding.sourceFunctions.length" class="text-[11px] text-muted">依据：{{
              aiMainFinding.sourceFunctions.join('、') }}</p>
          </div>
        </div>
      </div>
    ...
    <section v-else-if="aiSummaryMeta" class="card-surface space-y-3 p-6 text-sm text-secondary">
      <p class="text-lg font-semibold text-primary">AI 摘要暂未生成</p>
      <p>当前专题存在 AI 摘要文件，但未包含有效内容，请稍后刷新或重新运行分析。</p>
    </section>

    <section class="card-surface space-y-5 p-6" :id="analysisSections.length ? 'analysis-section-content' : undefined">
      <header class="flex flex-col gap-1">
        <h3 class="text-lg font-semibold text-primary">分析模块</h3>
      </header>
      <div v-if="analysisSections.length" class="space-y-5">
        <div class="flex flex-wrap gap-2">
          <button v-for="section in analysisSections" :key="section.name" type="button"
            ...
            :class="section.name === activeSectionName
                ? 'border-transparent bg-brand-soft/70 text-brand-700 ring-1 ring-brand-soft/80'
                : 'border-soft text-secondary hover:border-brand-soft hover:bg-surface-muted'
        ...
        <div v-if="activeSection" class="space-y-4 border-t border-soft pt-4">
          <header class="flex flex-wrap items-center justify-between gap-2 text-sm">
            <div class="flex items-center gap-2">
              ...
              <div class="space-y-0.5">
                <h3 class="text-base font-semibold leading-tight text-primary">{{ getChineseTitle(activeSection.label)
                  }}
                </h3>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <AnalysisChartPanel v-for="target in activeSection.targets" :key="`${activeSection.name}-${target.target}`"
              :title="target.title" :description="target.subtitle" :option="target.option" :has-data="target.hasData">
              <template #default>
                ...
                <details class="rounded-2xl border border-dashed border-soft px-3 py-2 text-xs">
                  ...
                  <pre
                    class="mt-2 max-h-56 overflow-auto bg-slate-900/90 p-3 text-brand-soft">{{ target.rawText }}</pre>
                </details>
              </template>
            </AnalysisChartPanel>
          </div>

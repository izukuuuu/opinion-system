    ...
    <section class="card-surface space-y-5 p-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="space-y-1">
          ...
          <p class="text-sm text-secondary">这里直接读取统一的结构化报告文档，章节、图表位点和引用索引都与正式报告共用同一套 schema。</p>
        </div>
      ...
      <section class="grid gap-6 xl:grid-cols-[1.1fr,0.9fr]">
        <article class="card-surface space-y-5 p-6">
          ...
          <div class="rounded-3xl border border-brand-soft bg-brand-soft/40 px-5 py-5">
            ...
            <p class="mt-3 text-sm leading-7 text-primary">{{ hero.summary || conclusion.executive_summary || '当前结果没有独立摘要。' }}</p>
            <div v-if="hero.risks?.length || conclusion.key_risks?.length" class="mt-3 flex flex-wrap gap-2">
              <span
                v-for="risk in (hero.risks?.length ? hero.risks : conclusion.key_risks || [])"
                :key="risk"
        ...
        <article class="card-surface space-y-4 p-6">
          <div class="space-y-1">
            ...
            <p class="text-sm text-secondary">结构化结果页和 AI 完整报告页现在使用同一套章节定义，目录、图表位置和引用锚点保持一致。</p>
          </div>
          ...
          <div v-if="hero.highlights?.length || conclusion.key_findings?.length" class="space-y-3">
            <article
              v-for="item in (hero.highlights?.length ? hero.highlights : conclusion.key_findings || [])"
              :key="item"
              class="rounded-3xl border border-soft bg-surface-muted/40 p-4"
            >
...
const chartCatalog = computed(() => (Array.isArray(report.value?.chart_catalog) ? report.value.chart_catalog : []))
...
const reportTitle = computed(() => hero.value.title || task.value.topic_label || task.value.topic_identifier || '结构化结果')
...
const summaryCards = computed(() => {
  ...
  return [
    ...
    { label: '图表位点', value: String(chartCatalog.value.length), detail: '来自现有分析结果' },

    ...
    <section class="card-surface space-y-6 p-6">
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        <article class="card-surface space-y-6 p-6">
          ...
          <form v-if="databaseFormVisible" class="space-y-4" @submit.prevent="submitDatabaseForm">
            ...
            <div class="flex flex-wrap gap-3">
              ...
              <button type="button" class="rounded-full border border-slate-200 px-5 py-2 text-sm font-semibold text-slate-600 hover:border-indigo-200 hover:text-indigo-600" @click="closeDatabaseForm">
                取消
              </button>
            </div>
          </form>
          ...
          <form class="space-y-4" @submit.prevent="submitLlmFilter">
            <div class="grid gap-4 md:grid-cols-2">
              <label class="flex flex-col gap-2 text-sm font-medium text-slate-600">
                <span>服务提供方</span>
                <input
                  v-model.trim="llmState.filter.provider"
                  type="text"
                  placeholder="如：qwen"
                  class="rounded-2xl border border-slate-300 bg-white px-3 py-2 text-sm focus:border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-100"
                />
          ...
          <form class="space-y-4" @submit.prevent="submitLlmAssistant">
            <div class="grid gap-4 md:grid-cols-2">
              <label class="flex flex-col gap-2 text-sm font-medium text-slate-600">
                <span>服务提供方</span>
                <input
                  v-model.trim="llmState.assistant.provider"
                  type="text"
...
const toggleDatabaseForm = () => {
  databaseFormVisible.value = !databaseFormVisible.value
...
const fetchDatabaseConnections = async () => {
  ...
  try {
    ...
    databaseState.connections = Array.isArray(data.connections) ? data.connections : []
...
const submitDatabaseForm = async () => {
  ...
  try {
    ...
    databaseState.message = '保存成功'
    await fetchDatabaseConnections()
    closeDatabaseForm()
  } catch (err) {
    databaseState.error = err instanceof Error ? err.message : '保存数据库连接失败'
  }
}
...
const fetchLlmSettings = async () => {
  ...
  try {
    ...
    const data = await response.json()
    Object.assign(llmState.filter, data.filter || {})
    Object.assign(llmState.assistant, data.assistant || {})
  } catch (err) {

  ...
  <div class="space-y-6 pb-12">
    <header class="flex flex-wrap items-center justify-between gap-4">
      <div class="space-y-1.5">
        <div class="flex items-center gap-3">
          ...
          <div
            class="inline-flex items-center gap-1.5 rounded-full bg-gradient-to-r from-brand-500 to-brand-600 px-3 py-1 text-[11px] font-semibold text-white">
            <AdjustmentsHorizontalIcon class="h-3.5 w-3.5" />
            <span>Filter</span>
          </div>
        </div>
        <p class="text-sm text-secondary">选定范围后，依次执行预清洗或 AI 筛选。</p>
      </div>
    </header>

    <section
      class="sticky top-[4.25rem] z-10 overflow-hidden rounded-3xl border border-brand-100/80 bg-white/95 backdrop-blur-md lg:top-[3.25rem]">
        ...
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div class="space-y-2">
            <div class="flex flex-wrap items-center gap-2">
              ...
              <span class="rounded-full px-2.5 py-1 text-[11px] font-semibold" :class="filterSourceTone">{{
                filterSourceLabel }}</span>
              <span class="text-xs text-secondary">优先动作：<span class="font-medium text-secondary">{{
                recommendedActionLabel }}</span></span>
            </div>
            <p class="text-sm text-primary">
              项目 <span class="font-semibold">{{ projectSummaryLabel }}</span>
              <span class="text-secondary"> · 处理范围 {{ cleanArchiveSummaryLabel }}</span>
            </p>
          </div>
          <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <div class="rounded-xl border border-indigo-100 bg-indigo-50/60 px-3 py-2">
              ...
              <p class="mt-0.5 text-sm font-bold text-indigo-700">{{ formatInteger(statusState.summary.token_usage || 0)
                }}</p>
            </div>
            <div class="rounded-xl border border-slate-100 bg-slate-50/80 px-3 py-2">
              <p class="text-[10px] font-medium uppercase tracking-wide text-muted">更新时间</p>
              <p class="mt-0.5 text-sm font-bold text-primary">{{ formatTimestamp(statusState.summary.updated_at) || '—'
                }}</p>
            </div>
          </div>
        </div>
    ...
    <section class="card-surface space-y-5 p-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        <div class="space-y-0.5">
          ...
          <p class="pl-3 text-sm text-secondary">选择项目与处理范围后，就可以统一维护排除词；保存后，预清洗和 AI 筛选都会使用。</p>
        </div>
        <button type="button"
          ...
          :disabled="projectsLoading" @click="refreshProjects">
      ...
      <div class="grid gap-5 xl:grid-cols-[1.05fr,1.3fr]">
        <div class="space-y-4 rounded-3xl border border-soft bg-surface-muted/60 p-5">
          <div>
            ...
            <p class="mt-1 text-sm text-secondary">指定项目和当前处理范围。</p>
          </div>
          ...
          <label class="space-y-2">
            ...
            <div v-if="cleanArchivesState.loading"
              class="rounded-2xl border border-soft bg-white px-4 py-3 text-sm text-secondary">
              正在同步可用范围…
            </div>
            <AppSelect
              v-else
              :options="cleanArchiveSelectOptions"
              :value="selectedCleanDate"
              :disabled="!cleanArchiveOptions.length"
              @change="selectedCleanDate = $event"
            />
            ...
            <div v-else class="space-y-2">
              ...
              <button v-if="showPreprocessCta" type="button"
        ...
        <div class="space-y-3">
          <div class="rounded-3xl border border-soft bg-surface-muted/60 p-5">
            ...
            <div
              class="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-soft bg-white px-4 py-4">
              <div class="space-y-1">
              ...
              <button type="button" class="btn-primary inline-flex items-center gap-2" :disabled="!currentProjectName"
                @click="openStopwordModal('pre')">
                打开排除词弹窗
              </button>
            </div>
            <p v-if="sharedPromptState.error" class="mt-3 text-xs text-danger">{{ sharedPromptState.error }}</p>
            <p v-else-if="sharedPromptState.success" class="mt-3 text-xs text-emerald-600">{{ sharedPromptState.success
              }}</p>
            ...
            <div class="mt-4 rounded-2xl border border-soft bg-white p-4">
              ...
              <div v-if="previewNoiseTerms.length" class="mt-3 flex max-h-52 flex-wrap gap-2 overflow-y-auto">
                <span v-for="term in previewNoiseTerms" :key="`noise-term-${term}`"
    ...
    <section class="card-surface space-y-5 p-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <TabSwitch :tabs="tabOptions" :active="activeTab" @change="activeTab = $event" />
      </div>
      ...
      <div v-if="activeTab === 'preclean'" class="grid gap-5 xl:grid-cols-3">
        <div class="space-y-4 rounded-3xl border border-brand-200 bg-brand-50/50 p-5 xl:col-span-2">
          <div>
            ...
            <h3 class="mt-2 text-base font-semibold text-primary">开始执行预清洗</h3>
          ...
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
            <div class="rounded-2xl border border-soft bg-white px-4 py-3">
              <p class="text-xs text-muted">适用场景</p>
              <p class="mt-1 text-sm font-semibold text-primary">预先排除明显噪声</p>
            </div>
            <div class="rounded-2xl border border-soft bg-white px-4 py-3">
              <p class="text-xs text-muted">排除词</p>
              <p class="mt-1 text-sm font-semibold text-primary">{{ parsedNoiseTerms.length }} 个</p>
            </div>
          </div>
          <div class="flex min-h-[3rem] flex-wrap items-center gap-3">
            <button type="button" class="btn-primary inline-flex items-center gap-2"
              :disabled="!canRunPreclean || precleanState.running" @click="runPreclean">
              <SparklesIcon class="h-4 w-4" />
              {{ precleanState.running ? '预清洗执行中…' : '开始执行预清洗' }}
            </button>
            <button v-if="showPreprocessCta && !canRunPreclean" type="button"
              class="btn-secondary inline-flex items-center gap-2" @click="goToPreprocess">
              <ArrowUpRightIcon class="h-4 w-4" />
              去做数据预处理
            </button>
            <p v-if="precleanState.message" class="text-xs"
              :class="precleanState.success === false ? 'text-danger' : 'text-secondary'">
      ...
      <div v-else-if="activeTab === 'ai'" class="grid gap-5 xl:grid-cols-[1.25fr,1fr]">
        <div class="space-y-4 rounded-3xl border border-soft bg-surface-muted/60 p-5">
          ...
          <label class="space-y-2">
            ...
            <textarea v-model="aiTemplateState.categoriesText" rows="5" class="input min-h-[9rem] resize-y"
              placeholder="支持换行、空格、逗号、中文逗号、分号。" />
          </label>

          <div class="grid gap-3 sm:grid-cols-2">
            <button type="button" class="btn-secondary" :disabled="aiTemplateState.loading || !currentProjectName"
              @click="loadAiTemplateConfig">
              {{ aiTemplateState.loading ? '读取中…' : '读取现有模板' }}
            </button>
            <button type="button" class="btn-primary" :disabled="aiTemplateState.saving || !canSaveAiTemplate"
              @click="saveAiTemplateConfig">
          ...
          <div class="rounded-3xl border border-soft bg-slate-950 px-4 py-4">
            ...
            <pre
              class="mt-3 max-h-72 overflow-y-auto whitespace-pre-wrap break-words text-xs leading-6 text-slate-200">{{ aiTemplatePreview }}</pre>
          </div>
        </div>
        ...
        <div class="space-y-4 rounded-3xl border border-soft bg-surface-muted/60 p-5">
          ...
          <button type="button" class="btn-primary inline-flex w-full items-center justify-center gap-2"
            :disabled="!canRunAiFilter || aiRunState.running" @click="runAiFilter">
            <SparklesIcon class="h-4 w-4" />
            {{ aiRunState.running ? 'AI 筛选运行中…' : '运行 AI 筛选' }}
          </button>
          <p v-if="aiRunState.message" class="text-xs"
            :class="aiRunState.success === false ? 'text-danger' : 'text-secondary'">
            {{ aiRunState.message }}
          </p>
        </div>
      </div>
    ...
    <section class="card-surface space-y-5 p-6">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <button type="button" class="btn-secondary inline-flex items-center gap-2"
          :disabled="statusLoading || !currentProjectName || !selectedCleanDate" @click="loadStatus">
      ...
      <div class="grid gap-3 md:grid-cols-4">
        ...
        <div class="rounded-2xl border border-indigo-100 bg-indigo-50/70 px-4 py-4">
          <p class="text-[10px] font-medium uppercase tracking-wide text-indigo-500/70">Token</p>
          <p class="mt-1.5 text-base font-bold text-indigo-700">{{ formatInteger(statusState.summary.token_usage || 0)
            }}</p>
        </div>
      </div>
      ...
      <div v-if="showResultEmptyState" class="rounded-3xl border border-dashed border-soft px-5 py-8 text-center">
        ...
        <button v-if="showPreprocessCta" type="button" class="btn-secondary mt-4 inline-flex items-center gap-2"
          @click="goToPreprocess">
          <ArrowUpRightIcon class="h-4 w-4" />
          去做数据预处理
        </button>
      ...
      <p v-else-if="statusState.message"
      ...
      <div v-if="!showResultEmptyState" class="grid gap-5 xl:grid-cols-[1.25fr,1fr]">
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            ...
            <span class="rounded-full bg-surface-muted px-2.5 py-1 text-[11px] font-semibold text-muted">{{
              statusState.recentRecords.length }} 条</span>
          </div>
          <div v-if="statusState.recentRecords.length" class="space-y-2.5">
            <article v-for="(record, index) in statusState.recentRecords"
              ...
              <div class="flex flex-wrap items-center justify-between gap-2">
                ...
                <span class="rounded-full px-2.5 py-1 text-[11px] font-semibold"
                  :class="record.status === 'discarded' ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'">
        ...
        <div class="space-y-3">
          <div class="rounded-3xl border border-emerald-100 bg-emerald-50/40 p-4">
            ...
            <div v-if="statusState.relevantSamples.length" class="mt-3 space-y-2">
              <article v-for="(item, index) in statusState.relevantSamples" :key="`relevant-${item.channel}-${index}`"
                class="rounded-2xl border border-emerald-100/70 bg-white px-4 py-3">
          ...
          <div class="rounded-3xl border border-rose-100 bg-rose-50/40 p-4">
            ...
            <div v-if="statusState.irrelevantSamples.length" class="mt-3 space-y-2">
              <article v-for="(item, index) in statusState.irrelevantSamples"
                :key="`irrelevant-${item.channel}-${index}`"
                class="rounded-2xl border border-rose-100/70 bg-white px-4 py-3">
                <p class="text-sm font-semibold text-primary">{{ item.title || '无标题记录' }}</p>
                <p class="mt-1 text-xs text-secondary">{{ item.preview || '—' }}</p>
              </article>
            </div>
    ...
    <AppModal v-model="stopwordModalOpen" title="项目排除词设置" description="左侧查看系统整理的高频词建议，右侧维护项目排除词。保存后，预清洗和 AI 筛选都会使用。"
      width="max-w-6xl" :show-footer="false" scrollable>
      <div class="space-y-4">
        <div class="rounded-3xl border border-soft bg-surface-muted/35 p-4">
          <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              />
              <input v-if="stopwordSuggestionUsesCustomTopK" v-model.trim="stopwordSuggestionCustomTopK" type="number"
                min="20" step="1" class="input w-28" placeholder="自定义" />
            </div>
          </div>
          ...
          <div class="mt-3 grid gap-2 lg:grid-cols-[minmax(0,1fr),auto,auto] lg:items-center">
            ...
            />
            <button type="button" class="btn-secondary inline-flex items-center justify-center gap-2 whitespace-nowrap"
              :disabled="stopwordSuggestionArchivesState.loading" @click="loadStopwordSuggestionArchives">
            ...
            <button type="button" class="btn-primary whitespace-nowrap"
              :disabled="stopwordSuggestionState.starting || !stopwordSuggestionCanRun"
              @click="startStopwordSuggestionTask(true)">
        ...
        <div class="grid gap-4 xl:grid-cols-[1.02fr,1.18fr]">
          <div class="rounded-3xl border border-soft bg-surface-muted/60 p-5">
            <div class="flex items-start justify-between gap-4">
              ...
              <div class="relative flex h-20 w-20 items-center justify-center">
                <svg class="absolute inset-0 h-full w-full -rotate-90" viewBox="0 0 80 80" aria-hidden="true">
                  <circle cx="40" cy="40" :r="stopwordSuggestionRingRadius" fill="none"
                    stroke="var(--color-surface-muted)" :stroke-width="stopwordSuggestionRingStroke" />
                  <circle v-if="stopwordSuggestionShowProgress" cx="40" cy="40" :r="stopwordSuggestionRingRadius"
                    fill="none" stroke="rgb(var(--color-brand-600) / 1)" :stroke-width="stopwordSuggestionRingStroke"
                    stroke-linecap="round" :stroke-dasharray="stopwordSuggestionRingDasharray" />
                </svg>
                <div
                  class="relative z-10 flex h-12 w-12 items-center justify-center rounded-full bg-surface text-sm font-semibold text-primary">
            ...
            <div class="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div class="rounded-2xl border border-soft bg-white px-4 py-3">
                ...
                <p class="mt-1 text-sm font-semibold text-primary">{{ stopwordSuggestionSummary.processed_docs || 0 }} /
                  {{ stopwordSuggestionSummary.total_docs || 0 }}</p>
              </div>
              <div class="rounded-2xl border border-soft bg-white px-4 py-3">
            ...
            <p v-if="stopwordSuggestionArchivesState.error" class="mt-3 text-xs text-danger">{{
              stopwordSuggestionArchivesState.error }}</p>
            <p v-if="stopwordSuggestionState.error" class="mt-2 text-xs text-danger">{{ stopwordSuggestionState.error }}
            </p>

            <div class="mt-4 space-y-2">
              <div v-if="stopwordSuggestionTerms.length" class="max-h-[28rem] space-y-2 overflow-y-auto pr-1">
                <button v-for="item in stopwordSuggestionTerms" :key="`suggestion-${item.term}`" type="button"
                  ...
                  :disabled="isNoiseTermSelected(item.term)" @click="appendNoiseTerm(item.term)">
                  <div>
                    <p class="text-sm font-semibold"
                      :class="isNoiseTermSelected(item.term) ? 'text-rose-700' : 'text-primary'">{{ item.term }}</p>
                    <p class="mt-1 text-xs"
                      :class="isNoiseTermSelected(item.term) ? 'text-rose-500' : 'text-secondary'">文档频次 {{
                        item.doc_count }} · 总词频 {{ item.total_count }}</p>
                  </div>
              ...
              <div v-else
                class="rounded-2xl border border-dashed border-soft bg-white px-4 py-6 text-sm text-secondary">
          ...
          <div class="rounded-3xl border border-soft bg-surface-muted/60 p-5">
            ...
            <div class="mt-4 flex gap-2">
              <input v-model.trim="manualNoiseTerm" type="text" class="input flex-1" placeholder="输入一个词后回车或点击追加"
                @keydown.enter.prevent="appendManualNoiseTerm" />
              <button type="button" class="btn-secondary" @click="appendManualNoiseTerm">追加</button>
            </div>

            <textarea v-model="sharedPromptState.projectStopwordsText" rows="12"
              class="input mt-4 min-h-[14rem] resize-y" placeholder="例如：油烟机 抽油烟机 厨电 方太 老板电器 ..." />
            ...
            <div class="mt-4 rounded-2xl border border-soft bg-white p-4">
              ...
              <div v-if="parsedNoiseTerms.length" class="mt-3 flex max-h-52 flex-wrap gap-2 overflow-y-auto">
                <div v-for="term in parsedNoiseTerms" :key="`modal-noise-term-${term}`"
                  ...
                  <button type="button"
                    ...
                    :aria-label="`移除 ${term}`" @click="removeNoiseTerm(term)">
...
const cleanArchiveSelectOptions = computed(() => {
  const placeholder = { value: '', label: cleanArchiveOptions.value.length ? '请选择处理范围' : '暂无可用范围', disabled: true }
  return [placeholder, ...cleanArchiveOptions.value]
})
...
const recommendedActionLabel = computed(() => {
  ...
  if (!statusState.summary.total_rows) return '开始执行预清洗'
  if ((statusState.source || statusState.summary.source || '') === 'keyword-preclean') return '如需进一步细筛，可运行 AI 筛选'
  if ((statusState.source || statusState.summary.source || '') === 'ai-filter') return '结果已生成，可查看记录或继续处理'
  return '开始执行预清洗'
})
...
const resultEmptyDescription = computed(() => {
  if (!currentProjectName.value) return '选定项目后，页面会自动读取排除词和可用的处理范围。'
...
async function refreshProjectContext() {
  await Promise.all([loadAiTemplateConfig(), loadSharedPromptConfig(), loadCleanArchives()])
  await loadStatus()
}

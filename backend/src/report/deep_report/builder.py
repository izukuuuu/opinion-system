...
若 evidence_cards.result 为空，生成包含 6 个空结构的标准化结果后立即结束：
  { "status": "empty", "reason": "上游证据为空", "result": { "event_trigger": {}, "discussion_evolution": {}, "actor_distribution": {}, "platform_analysis": {}, "keywords": {}, "sentiment_summary": {} }, "skipped_due_to": ["upstream_empty"] }
...
必须把对应的 skipped_due_to 值写入 result.missing_dimensions，并将 decision 设为 'fallback_recompile' 或 'require_semantic_review'。
...
- 禁止在没有证据支撑的情况下下结论
- 禁止把 `require_semantic_review` 理解成“停止产出”；它表示内容仍可编译，但后续必须进入自动重写或人工审查。
""",
            "tools": writer_tools,

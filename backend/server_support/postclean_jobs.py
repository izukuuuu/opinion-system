...
用于在内存中跟踪后清洗任务的运行状态、心跳、简要日志与结果，
供前端轮询展示统一的 worker / heartbeat 视图。
"""
...
def create_postclean_job(topic: str, database: str, tables: Optional[Sequence[str]] = None) -> Dict[str, Any]:
...
def get_postclean_job(topic: str, database: str, tables: Optional[Sequence[str]] = None) -> Optional[Dict[str, Any]]:
    key = _job_key(topic, database, tables)
    with _lock:
...
def list_postclean_jobs() -> List[Dict[str, Any]]:
    with _lock:
        return [dict(payload) for payload in _jobs.values()]


def update_postclean_job(
    topic: str,
    database: str,
...
def heartbeat_postclean_job(topic: str, database: str, tables: Optional[Sequence[str]] = None) -> Dict[str, Any]:
    key = _job_key(topic, database, tables)
    with _lock:
        payload = _jobs.get(key)
        if payload is None:

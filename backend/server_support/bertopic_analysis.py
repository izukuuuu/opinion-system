"""BERTopic background task queue management."""
...
STATE_ROOT = get_data_root() / "_bertopic_analysis"
...
def _update_task(task_id: str, mutate: Callable[[Dict[str, Any]], None]) -> Dict[str, Any]:
    ...
    with lock:
        ...
        if not isinstance(task, dict) or not task.get("id"):
            raise LookupError("未找到 BERTopic 任务")
        mutate(task)
        task["updated_at"] = _utc_now()
        _atomic_write_json(path, task)
        return task
...
def _new_task_id() -> str:
    return f"bt-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid4().hex[:6]}"
...
def create_task(
    *,
    ...
    task = {
        ...
        "display_topic": str(display_topic or topic_identifier or "").strip(),
...
def reserve_next_task() -> Optional[Dict[str, Any]]:
    ...
    return mark_task_progress(
        ...
        phase="prepare",
        percentage=1,
        message="worker 已接单，正在准备 BERTopic 分析。",
    )
...
def ensure_worker_running() -> Dict[str, Any]:
    ...
    worker_script = Path(__file__).resolve().parent / "bertopic_analysis_worker.py"
    ...
    try:
        ...
        _atomic_write_json(WORKER_STATUS_PATH, status)
    LOGGER.info("bertopic worker started | pid=%s", process.pid)
    return status

"""Fluid analysis background task queue management."""
...
STATE_ROOT = get_data_root() / "_fluid_analysis"
...
def _new_task_id() -> str:
    return f"fi-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid4().hex[:6]}"
...
def create_task(
    ...
    *,
    ...
    task = {
        ...
        "window_hours": max(_safe_int(window_hours, 3), 1),
        "target_file": str(target_file or "").strip() if target_file else "",
        "status": "queued",
        "phase": "queued",
        "percentage": 0,
        "message": "等待流体指标 worker 接单。",
        "worker_pid": 0,
...
def create_or_reuse_task(
    ...
    *,
    ...
    task = create_task(
        topic_identifier, start_date, end_date=end_date, window_hours=window_hours, target_file=target_file
    )
...
def ensure_worker_running() -> Dict[str, Any]:
    ...
    with worker_lock:
        ...
        worker_script = Path(__file__).resolve().parent / "fluid_indicator_worker.py"
        ...
        _atomic_write_json(WORKER_STATUS_PATH, status)
        LOGGER.info("fluid indicator worker started | pid=%s", process.pid)
        return status
